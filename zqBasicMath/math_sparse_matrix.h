...
*/
/***********************************************************/
#ifndef __MATH_SPARSE_MATRIX_H__
#define __MATH_SPARSE_MATRIX_H__
...
public:
	...
	*/
	...
	/**
		return the cols of a matrix
	*/
	virtual inline SparseVector<T> GetSparseCol(int i) const { throw "incomplete"; }
	
	/**
		return the row of a matrix
	*/
	virtual inline SparseVector<T> GetSparseRow(int i)  const { throw "incomplete"; }

	/**
		return the cols of a matrix
	*/
	virtual inline void SetSparseCol(const SparseVector<T>& vec, int i) { throw "incomplete"; }

	/**
		return the row of a matrix
	*/
	virtual inline void SetSparseRow(const SparseVector<T>& vec, int i) { throw "incomplete"; }
...
//	========================================
/**
	Linked List Matrix format
*/
//	========================================
template<class T>
class SparseMatrixLIL : public SparseMatrixBase<T> {
public:
	using SparseMatrixBase<T>::row_num;
	using SparseMatrixBase<T>::col_num;
	using SparseMatrixBase<T>::indexing;
...
public:
	SparseMatrixLIL(int m = 0, int n = 0) : SparseMatrixBase<T>(m, n) {
		value.resize(m);
		rows.resize(m);
	}
	...
	inline void SetIndexing(int idxing) {
		if (!indexing && idxing) {		//	change from zero to one
			for (int i = 0; i < rows.size(); i++) {
				auto iter = rows[i].begin();
				while (iter != rows[i].end()) {
					(*iter)++;
					iter++;
				}
			}
	...
	*/
	inline void InsertElement(T val, int m, int n) {
		assert(m >= 0 && m < row_num&& n >= 0 && n < col_num);	//	legal position
		assert(!CheckExist(m, n));					//	the element doesn't exist
		auto row_iter = rows[m].begin();
		auto value_iter = rows[m].begin();
		bool find = false;
		for (; row_iter != rows[m].end(); row_iter++) {
			if ((row_iter == rows[m].begin() || *(std::prev(row_iter,1)) < n) &&
				*(row_iter) > n) {
				rows[m].insert(row_iter, n);
				value[m].insert(value_iter, val);
				find = true;
				break;
			}
		}
		if (!find) {
			rows[m].push_back(n);
			value[m].push_back(val);
		}
	}

	/**
		SparseMatrixLIL is not symmetric structure
	*/
	inline int IsSymmetric() const {
		return 0;
	}
	...
	*/
	...
	/**
		Return transposed matrix, but don't change old data
	*/
	inline SparseMatrixLIL Transpose() const {
		SparseMatrixLIL result(*this);
		result.SetTranspose();
		return result;
	}
	/**
		Set the matrix to be its transpose
	*/
	inline SparseMatrixLIL& SetTranspose() {
		return *(this);
	}
	...
	/**
		return the row of a matrix
	*/
	inline SparseVector<T> GetSparseRow(int i) const {
		SparseVector<T> result(col_num);
		auto value_iter = value[i].begin();
		auto row_iter = rows[i].begin();
		for (; value_iter != value[i].end(); value_iter++, row_iter++) {
			result[*(row_iter)] = *value_iter;
		}
		return result;
	}
	...
	/**
		return the row of a matrix
	*/
	inline void SetSparseRow(const SparseVector<T>& vec,int i) {
		assert(vec.dim_num == col_num);
		value[i]=vec.value;
		rows[i]=vec.index;
		if (indexing == 1) {
...
public:
	static SparseMatrixLIL<T> IndentityMatrix(int rows, int cols) {
		SparseMatrixLIL<T> m(rows, cols);
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < cols; j++) {
				if (i == i) {
					m.InsertElement(One<int>(), i, j);
				}
			}
		}
		return m;
...
protected:
	...
	*/
	inline int CheckExist(int m, int n) {
		assert(m >= 0 && m < row_num);
		auto iter = rows[m].begin();
		for (; iter != rows[m].end(); iter++) {
			if(*iter==n)
				return 1;
		}
		return 0;
...
template<class T> inline std::ostream& operator << (std::ostream& stream, const SparseMatrixLIL<T>& rhs) {
	for (int i = 0; i < rhs.row_num; i++) {
		std::cout << rhs.GetSparseRow(i);
		if(i!=rhs.row_num-1)
			std::cout<< std::endl;
	}
	return stream;
